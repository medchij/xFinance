...
const COMPANY_ID = 'dataMAC';

const ENTITIES = {
  Settings: { table: 'settings', file: 'Settings.json' },
};

async function populateDataMAC() {
  const client = await db.connect();
  
  try {
    console.log(`\n--- dataMAC компани өгөгдөл нэмэх эхэллээ ---`);
    
    // 1. Компани үүсгэх (хэрэв байхгүй бол)
    await client.sql`
      INSERT INTO companies (id, name)
      VALUES (${COMPANY_ID}, 'dataMAC')
    ...
    console.log('\n✅ dataMAC компани өгөгдөл амжилттай нэмэгдлээ!');
  } catch (error) {
    console.error('❌ Populate dataMAC алдаа:', error);
    throw error;
  } finally {
    client.release();
  }
